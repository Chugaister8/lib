...
/* ==================
   RENDER MEASURE BLOCK
   ================== */
const renderMeasureBlock = (measure, riskIndex, measureIndex) => `
  <div class="thematic-measure-block"
    id="measure-block-${riskIndex}-${measureIndex}"
    data-risk-index="${riskIndex}"
    data-measure-index="${measureIndex}">

    <div class="thematic-measure-block__header">
      <p class="thematic-measure-block__title">
        <span class="material-symbols-rounded">task_alt</span>
        Захід ${measureIndex + 1}
        ${measure.id ? `<span class="thematic-risk-block__id">${measure.id}</span>` : ''}
      </p>
      <button
        type="button"
        class="btn btn--ghost btn--icon btn--sm"
        data-action="remove-measure"
        data-risk-index="${riskIndex}"
        data-measure-index="${measureIndex}"
    ...
    <div class="risk-form__grid">
      ...
      <div class="risk-form__field risk-form__field--full">
        ...
        <input
          ...
          name="risk_${riskIndex}_measure_${measureIndex}_title"
          value="${measure.title || ''}"
          placeholder="Назва заходу впливу на ризик"
          required
        />
      </div>
      ...
      <div class="risk-form__field risk-form__field--full">
        ...
        <textarea
          ...
          name="risk_${riskIndex}_measure_${measureIndex}_desc"
          placeholder="Детальний опис заходу"
          rows="2"
        >${measure.desc || ''}</textarea>
      </div>
      ...
      <div class="risk-form__field">
        ...
        <input
          type="text"
          class="risk-form__input"
          name="risk_${riskIndex}_measure_${measureIndex}_responsible"
      ...
      <div class="risk-form__field">
        ...
        <input
          type="date"
          class="risk-form__input"
          name="risk_${riskIndex}_measure_${measureIndex}_deadline"
...
const renderRiskBlock = (risk, index) => `
  <div class="thematic-risk-block" data-risk-index="${index}" id="risk-block-${index}">
    ...
    <!-- Існуючі заходи контролю + метод -->
    <div class="thematic-risk-block__section">
      <p class="thematic-risk-block__section-title">Контроль та методи впливу</p>
      <div class="risk-form__grid">
    ...
    <!-- Заходи мінімізації -->
    <div class="thematic-risk-block__section">
      <p class="thematic-risk-block__section-title">Заходи мінімізації ризику</p>

      <div id="measures-list-${index}">
        ${(risk.measures || []).map((m, mi) => renderMeasureBlock(m, index, mi)).join('')}
      </div>
      ...
      <div class="modal modal--xl">
        ...
        <div class="modal__footer modal__footer--between">
          ...
          <div style="display:flex;gap:var(--spacing-sm)">
            ...
            <button class="btn btn--ghost" id="thematic-form-draft">
              <span class="material-symbols-rounded">save</span>
              Зберегти чернетку
            </button>
            <button class="btn btn--secondary" id="thematic-form-pending">
...
/* ==================
   ...
   ================== */
const collectMeasureData = (form, riskIndex, measureIndex, existingId) => {
  const fd = new FormData(form);
  return {
    id:          existingId || generateThematicMeasureId(),
    title:       fd.get(`risk_${riskIndex}_measure_${measureIndex}_title`)       || '',
    desc:        fd.get(`risk_${riskIndex}_measure_${measureIndex}_desc`)        || '',
    responsible: fd.get(`risk_${riskIndex}_measure_${measureIndex}_responsible`) || '',
    deadline:    fd.get(`risk_${riskIndex}_measure_${measureIndex}_deadline`)    || '',
  };
};
...
const collectRiskData = (form, index) => {
  ...
  const measures = (formState.risks[index]?.measures || [])
    .map((m, mi) => collectMeasureData(form, index, mi, m.id));
...
const validateForm = (form) => {
  ...
  formState.risks.forEach((risk, i) => {
    ...
    if (measures.length === 0) {
      errors.push(`Ризик ${i + 1}: додайте хоча б один захід мінімізації`);
    } else {
      measures.forEach((_, mi) => {
        if (!fd.get(`risk_${i}_measure_${mi}_title`))       errors.push(`Ризик ${i + 1}, Захід ${mi + 1}: введіть назву`);
        if (!fd.get(`risk_${i}_measure_${mi}_responsible`)) errors.push(`Ризик ${i + 1}, Захід ${mi + 1}: введіть відповідальний підрозділ`);
        if (!fd.get(`risk_${i}_measure_${mi}_deadline`))    errors.push(`Ризик ${i + 1}, Захід ${mi + 1}: вкажіть строк`);
      });
    }
  });
...
const updateScorePreview = (form, index) => {
  ...
  if (preview) {
    ...
    newPreview.innerHTML = renderScorePreview(index, probability, financialImpact, nonFinancialImpact);
    preview.replaceWith(newPreview.firstElementChild);
  }
};
...
const rerenderRisksList = (form) => {
  ...
  if (!list) return;
  list.innerHTML = formState.risks.map((r, i) => renderRiskBlock(r, i)).join('');
  formState.risks.forEach((_, i) => bindRiskBlockEvents(form, i));
};

/* ==================
   RERENDER MEASURES LIST
   ================== */
const rerenderMeasuresList = (form, riskIndex) => {
  const list = document.getElementById(`measures-list-${riskIndex}`);
  if (!list) return;
  const measures = formState.risks[riskIndex]?.measures || [];
  list.innerHTML = measures.map((m, mi) => renderMeasureBlock(m, riskIndex, mi)).join('');
  measures.forEach((_, mi) => bindMeasureBlockEvents(form, riskIndex, mi));
};

/* ==================
   BIND MEASURE EVENTS
   ================== */
const bindMeasureBlockEvents = (form, riskIndex, measureIndex) => {
  const block = document.getElementById(`measure-block-${riskIndex}-${measureIndex}`);
  if (!block) return;

  block.querySelector(`[data-action="remove-measure"][data-risk-index="${riskIndex}"][data-measure-index="${measureIndex}"]`)
    ?.addEventListener('click', () => {
      formState.risks[riskIndex].measures.splice(measureIndex, 1);
      rerenderMeasuresList(form, riskIndex);
    });
};
...
const bindRiskBlockEvents = (form, index) => {
  ...
  // Add measure
  block.querySelector(`[data-action="add-measure"][data-risk-index="${index}"]`)
    ?.addEventListener('click', () => {
      if (!formState.risks[index].measures) {
        formState.risks[index].measures = [];
      }
      formState.risks[index].measures.push({});
      rerenderMeasuresList(form, index);
    });

  // Bind existing measures
  const measures = formState.risks[index]?.measures || [];
  measures.forEach((_, mi) => bindMeasureBlockEvents(form, index, mi));
};
...
const bindFormEvents = (onSubmit) => {
  ...
  document.getElementById('add-thematic-risk')
    ?.addEventListener('click', () => {
      formState.risks.push({ measures: [] });
      const list  = document.getElementById('thematic-risks-list');
      const index = formState.risks.length - 1;
      if (list) {
        list.insertAdjacentHTML('beforeend', renderRiskBlock({ measures: [] }, index));
  ...
  // Submit for review
  document.getElementById('thematic-form-pending')
    ?.addEventListener('click', () => {
  ...
  document.getElementById('thematic-form-submit')
    ?.addEventListener('click', () => {
      const data = collectFormData(form, formState.currentStatus || 'DRAFT');
      onSubmit(data);
      removeModal();
    });
};
...
export const openThematicAssessmentForm = (onSubmit, data = {}) => {
  ...
  formState.risks         = data.risks
    ? data.risks.map(r => ({ ...r, measures: r.measures || [] }))
    : [];
  formState.currentStatus = data.status || 'DRAFT';
